...
const createExecuteAggregation = (dbName) => {
    async function aggregate(dbName, collection, pipeline) {
        const db = client.db(dbName);
        const col = db.collection(collection);
    ...
    return (collection, pipeline) => aggregate(dbName, collection, pipeline);
};
...
const pipeline = [
  ...
  { $group: {
      ...
      totalTopicStars: { $sum: "$totalStars" }
  }},
  { $addFields: {
      dependencies: {
        $sortArray: {
  ...
  { $sort: { totalTopicStars: -1 } }
];

const aggregateResults = () => createExecuteAggregation(dbName)("topics",pipeline)
const saveBenchmark = (data) => insertData("benchmarks",data)
...
export default {insertData, readData, findFirst, dropCollection, insertBatch, updateData, aggregateResults,saveBenchmark}
